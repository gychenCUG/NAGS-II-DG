...
def process_sample(i, source_props, R2, V, D):
    ...
    for j in range(Cepoch):
        ...
        source_4_sam = source_4.sample(temp[3], replace=False)
        source_5_sam = source_5.sample(temp[4], replace=False)
        source_6_sam = source_6.sample(temp[5], replace=False)
        source_7_sam = source_7.sample(temp[6], replace=False)
        source_8_sam = source_8.sample(temp[7], replace=False)
        source_9_sam = source_9.sample(temp[8], replace=False)
        source_10_sam = source_10.sample(temp[9], replace=False)
...
if __name__ == '__main__':
    ...
    for i in range(model_num):
        ...
        source_props = np.zeros((num_samples, 10))
