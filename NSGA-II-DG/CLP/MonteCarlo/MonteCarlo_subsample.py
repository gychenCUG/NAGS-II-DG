...
def process_sample(i, source_props, R2, V, D):
    ...
    for j in range(Cepoch):
        ...
        # subsampling
        gobi_sam = gobi.sample(temp[0], replace=False)
        yr_sam = yr.sample(temp[1], replace=False)
        ntp_sam = ntp.sample(temp[2], replace=False)
        ncc_sam = ncc.sample(temp[3], replace=False)
        csd_sam = csd.sample(temp[4], replace=False)
        qaidam_sam = qaidam.sample(temp[5], replace=False)
        qilian_sam = qilian.sample(temp[6], replace=False)
...
def save(model_num, source_props, R2, V, D):
    ...
    # Save all data
    df.to_csv(f'model_{model_num}_{potName}_all.csv', index=False)
    ...
    # Save results
    df_r2_head.to_csv(f'model_{model_num}_{potName}_r2head.csv', index=False)
    df_v_head.to_csv(f'model_{model_num}_{potName}_vhead.csv', index=False)
    df_d_head.to_csv(f'model_{model_num}_{potName}_dhead.csv', index=False)
...
if __name__ == '__main__':
    ...
    for i in range(model_num):
        ...
        source_props = np.zeros((num_samples, 7))
